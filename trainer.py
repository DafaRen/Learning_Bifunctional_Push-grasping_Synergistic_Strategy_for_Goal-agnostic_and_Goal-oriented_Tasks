...
from models import reinforcement_net,goal_grasp_net
...
class Trainer(object):
    def __init__(self, method, push_rewards, future_reward_discount,
        ...
        if self.method == 'reinforcement':
            ...
            self.goal_criterion = torch.nn.SmoothL1Loss(reduce=False) # Huber loss
            if self.use_cuda:
                self.goal_criterion = self.goal_criterion.cuda()
        ...
        self.goal_label_value_log = []
        self.goal_reward_value_log = []
        self.goal_predicted_value_log = []
    ...
    def preload(self, transitions_directory):
        ...
        self.goal_predicted_value_log = np.loadtxt(os.path.join(transitions_directory, 'goal-predicted-value.log.txt'), delimiter=' ')
        self.goal_predicted_value_log = self.goal_predicted_value_log[0:self.iteration]
        self.goal_predicted_value_log.shape = (self.iteration,1)
        self.goal_predicted_value_log = self.goal_predicted_value_log.tolist()
    ...
    def get_label_value(self, primitive_action, push_success, grasp_success, change_detected, push_effective, prev_push_predictions, prev_grasp_predictions, next_color_heightmap, next_depth_heightmap):
        ...
        if self.method == 'reinforcement':
            ...
            if not change_detected and not grasp_success and not push_effective:
                future_reward = 0
            else:
