...
class reinforcement_net(nn.Module):
    ...
    def __init__(self, use_cuda): # , snapshot=None
        ...
        self.color_trunk = dense.densenet121(pretrained=True)
        self.depth_trunk = dense.densenet121(pretrained=True)
        ...
        self.grasp_conv16 = nn.Sequential(OrderedDict([
            ('grasp-conv16-norm0', nn.BatchNorm2d(2048)),
            ('grasp-conv16-relu0', nn.ReLU(inplace=True)),
            ('grasp-conv16-conv0', nn.Conv2d(2048, 1024, kernel_size=1, stride=1, bias=False))
            # ('grasp-upsample2', nn.Upsample(scale_factor=4, mode='bilinear'))
        ]))
        self.grasp_up16sto4s=nn.UpsamplingBilinear2d(scale_factor=4)
        self.grasp_conv4 = nn.Sequential(OrderedDict([
            ('grasp-conv4-norm0', nn.BatchNorm2d(2048)),
            ('grasp-conv4-relu0', nn.ReLU(inplace=True)),
            ('grasp-conv4-conv0', nn.Conv2d(2048, 64, kernel_size=1, stride=1, bias=False))
            
        ]))
        self.grasp_up4sto1s=nn.UpsamplingBilinear2d(scale_factor=4)
        ...
        self.push_conv16 = nn.Sequential(OrderedDict([
            ('push-conv16-norm0', nn.BatchNorm2d(2048)),
            ('push-conv16-relu0', nn.ReLU(inplace=True)),
            ('push-conv16-conv0', nn.Conv2d(2048, 1024, kernel_size=1, stride=1, bias=False))
            
        ]))
        self.push_up16sto4s=nn.UpsamplingBilinear2d(scale_factor=4)
        self.push_conv4 = nn.Sequential(OrderedDict([
            ('push-conv4-norm0', nn.BatchNorm2d(2048)),
            ('push-conv4-relu0', nn.ReLU(inplace=True)),
            ('push-conv4-conv0', nn.Conv2d(2048, 64, kernel_size=1, stride=1, bias=False))
            
        ]))
        self.push_up4sto1s=nn.UpsamplingBilinear2d(scale_factor=4)
        # Initialize network weights
        for m in self.named_modules():
        ...
        if is_volatile:
            with torch.no_grad():
                ...
                for rotate_idx in range(self.num_rotations):
                    ...
                    interm_color_feat_x4s,interm_color_feat_x16s = self.color_trunk(rotate_color)
                    interm_depth_feat_x4s,interm_depth_feat_x16s = self.depth_trunk(rotate_depth)
                    interm_feat_x16s = torch.cat((interm_color_feat_x16s, interm_depth_feat_x16s), dim=1)
                    interm_grasp_feat=self.grasp_conv16(interm_feat_x16s)
                    interm_grasp_feat=self.grasp_up16sto4s(interm_grasp_feat)
                    interm_grasp_feat_x4s = torch.cat((interm_color_feat_x4s, interm_depth_feat_x4s), dim=1)
                    interm_grasp_feat_x4s = torch.cat((interm_grasp_feat_x4s, interm_grasp_feat), dim=1)
                    interm_grasp_feat=self.grasp_conv4(interm_grasp_feat_x4s)
                    interm_grasp_feat=self.grasp_up4sto1s(interm_grasp_feat)
                    
                    interm_push_feat=self.push_conv16(interm_feat_x16s)
                    interm_push_feat=self.push_up16sto4s(interm_push_feat)
                    interm_push_feat_x4s = torch.cat((interm_color_feat_x4s, interm_depth_feat_x4s), dim=1)
                    interm_push_feat_x4s = torch.cat((interm_push_feat_x4s, interm_push_feat), dim=1)
                    interm_push_feat=self.push_conv4(interm_push_feat_x4s)
                    interm_push_feat=self.push_up4sto1s(interm_push_feat)
                    ...
                    graspnet_feat = self.graspnet(interm_grasp_feat)
                    grasp_q=graspnet_feat
                    pushnet_feat = self.pushnet(interm_push_feat)
                    push_q=pushnet_feat
                    output_prob.append([F.grid_sample(push_q, flow_grid_after, mode='nearest'),
                                        F.grid_sample(grasp_q, flow_grid_after, mode='nearest')])
            ...
            pushnet_feat = self.pushnet(interm_push_feat)
            push_q=pushnet_feat
            self.output_prob.append([F.grid_sample(push_q, flow_grid_after, mode='nearest'),
                                     F.grid_sample(grasp_q, flow_grid_after, mode='nearest')])
...
class goal_grasp_net(nn.Module):

    def __init__(self, use_cuda): # , snapshot=None
        super(goal_grasp_net, self).__init__()
        self.use_cuda = use_cuda
