...
class Trainer(object):
    def __init__(self, method, push_rewards, future_reward_discount,
        ...
        # Fully convolutional Q network for deep reinforcement learning
            ...
            self.model_tg = reinforcement_net(self.use_cuda)
            self.model_tg.load_state_dict(self.model.state_dict())
            self.model_tg.eval()
    ...
    def forward(self, color_heightmap, depth_heightmap, is_volatile=False, specific_rotation=-1):
        ...
        return push_predictions, grasp_predictions, output_prob
    def forward_tg(self, color_heightmap, depth_heightmap, is_volatile=False, specific_rotation=-1):
        ...
        # Pass input data through model
        output_prob_tg, state_feat_tg = self.model_tg.forward(input_color_data, input_depth_data, is_volatile, specific_rotation)

        if self.method == 'reactive':

            # Return affordances (and remove extra padding)
            for rotate_idx in range(len(output_prob_tg)):
                if rotate_idx == 0:
                    push_predictions = F.softmax(output_prob_tg[rotate_idx][0], dim=1).cpu().data.numpy()[(padding_width/2):(color_heightmap_2x.shape[0]/2 - padding_width/2),(padding_width/2):(color_heightmap_2x.shape[0]/2 - padding_width/2)]
                    grasp_predictions = F.softmax(output_prob_tg[rotate_idx][1], dim=1).cpu().data.numpy()[(padding_width/2):(color_heightmap_2x.shape[0]/2 - padding_width/2),(padding_width/2):(color_heightmap_2x.shape[0]/2 - padding_width/2)]
                else:
                    push_predictions = np.concatenate((push_predictions, F.softmax(output_prob_tg[rotate_idx][0], dim=1).cpu().data.numpy()[(padding_width/2):(color_heightmap_2x.shape[0]/2 - padding_width/2),(padding_width/2):(color_heightmap_2x.shape[0]/2 - padding_width/2)]), axis=0)
                    grasp_predictions = np.concatenate((grasp_predictions, F.softmax(output_prob_tg[rotate_idx][1], dim=1).cpu().data.numpy()[(padding_width/2):(color_heightmap_2x.shape[0]/2 - padding_width/2),(padding_width/2):(color_heightmap_2x.shape[0]/2 - padding_width/2)]), axis=0)
            ...
            for rotate_idx in range(len(output_prob_tg)):
                if rotate_idx == 0:
                    push_predictions = output_prob_tg[rotate_idx][0].cpu().data.numpy()[:,0,int(padding_width/2):int(color_heightmap_2x.shape[0]/2 - padding_width/2),int(padding_width/2):int(color_heightmap_2x.shape[0]/2 - padding_width/2)]
                    grasp_predictions = output_prob_tg[rotate_idx][1].cpu().data.numpy()[:,0,int(padding_width/2):int(color_heightmap_2x.shape[0]/2 - padding_width/2),int(padding_width/2):int(color_heightmap_2x.shape[0]/2 - padding_width/2)]
                else:
                    push_predictions = np.concatenate((push_predictions, output_prob_tg[rotate_idx][0].cpu().data.numpy()[:,0,int(padding_width/2):int(color_heightmap_2x.shape[0]/2 - padding_width/2),int(padding_width/2):int(color_heightmap_2x.shape[0]/2 - padding_width/2)]), axis=0)
                    grasp_predictions = np.concatenate((grasp_predictions, output_prob_tg[rotate_idx][1].cpu().data.numpy()[:,0,int(padding_width/2):int(color_heightmap_2x.shape[0]/2 - padding_width/2),int(padding_width/2):int(color_heightmap_2x.shape[0]/2 - padding_width/2)]), axis=0)

            
        return push_predictions, grasp_predictions, state_feat_tg
    ...
    def get_label_value(self, primitive_action, push_success, grasp_success, change_detected, prev_push_predictions, prev_grasp_predictions, next_color_heightmap, next_depth_heightmap):
        ...
        if self.method == 'reactive':
            ...
            if not change_detected and not grasp_success:
                ...
                next_push_predictions0, next_grasp_predictions0, next_state_feat0 = self.forward(next_color_heightmap, next_depth_heightmap, is_volatile=True)
                next_push_predictions, next_grasp_predictions, next_state_feat = self.forward_tg(next_color_heightmap, next_depth_heightmap, is_volatile=True)
                max_action=np.unravel_index(np.argmax(next_push_predictions0), next_push_predictions0.shape)
                max_action1=np.unravel_index(np.argmax(next_grasp_predictions0), next_grasp_predictions0.shape)
                future_reward = max(next_push_predictions[max_action],next_grasp_predictions[max_action1]) 
