...
interval_size = 100 # Report performance over the last 200 training steps
...
colors = [[089.0/255.0,169.0/255.0,079.0/255.0], # green
...
max_plot_iteration = min(executed_action_log.shape[0] - 2, max_plot_iteration)
# Create plot design
plt.ylim((0, 1))
...
for session_idx in range(len(session_directories)):
    ...
    for step in range(max_iteration):
        ...
        if method == 'reactive':
            ...
            grasp_success_over_interval = np.sum(reward_value_log[grasp_attempt_ind_over_interval] > 0)/float(min(interval_size,max(step,1))) # Reward value for successful grasping is anything larger than 0.5 (reinforcement policy)
        ...
        push_then_grasp_success[step] = 0.9
    ...
    plt.plot(range(0, max_iteration), push_then_grasp_success, dashes=[8,7], color='gray', linewidth=1, alpha=1, dash_capstyle='round', dash_joinstyle='round',label='_nolegend_') # color='blue', dashes=[5,5], linewidth=2, dash_capstyle='butt')
