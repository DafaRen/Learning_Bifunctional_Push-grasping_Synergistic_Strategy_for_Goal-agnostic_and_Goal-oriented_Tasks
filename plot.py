...
for session_idx in range(len(session_directories)):
    ...
    push_then = np.zeros((max_iteration))
    ...
    #plt.fill_between(range(0, max_iteration), push_then_grasp_success,  dashes=[8,7], color=color, alpha=0.5)
    plt.plot(range(0, max_iteration), push_then, dashes=[8,7], color='gray', linewidth=1, alpha=1, dash_capstyle='round', dash_joinstyle='round',label='_nolegend_') # color='blue', dashes=[5,5], linewidth=2, dash_capstyle='butt')
